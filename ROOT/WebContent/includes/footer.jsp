...
<%if(request.getAttribute("script") != null){ %>
	<script type="text/javascript">$(function(){localStorage.removeItem("venta");})</script>
<% } else { %>
	...
	$(function(){
		...
		$("select[name=tipoFactura] option[value="+oldVta.tipoFactura+"]").attr('selected', 'selected');
		$("select[name=medioPago] option[value="+oldVta.medioPago+"]").attr('selected', 'selected');
		...
		$('input[name=debitoCodigoSeguridad]').val(oldVta.debitoCodigoSeguridad);
		...
		$('input[name=creditoCodigoSeguridad]').val(oldVta.creditoCodigoSeguridad);
		...
		$.each(oldVta.items, function(i,v){
			var itemVta = $('<tr><th scope="row">'+index+'</th><td>'+v.codigo+'</td><td>'+v.nombre+'</td><td>'+v.presentacion+'</td><td>'+v.cantidad+'</td><td>$'+v.precio.toFixed(2)+'</td><td>$'+(v.precio.toFixed(2)*v.cantidad).toFixed(2)+'</td><td><a href="#" class="delete"><i class="fas fa-trash text-danger"></i></a></td><input type="hidden" name="items" value="'+v.codigo+','+v.cantidad+'" /></tr>')
